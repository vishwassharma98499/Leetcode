#The code implements the A* algorithm to find the shortest path on a grid with obstacles.
#  It uses a priority queue to explore nodes based on cost and a heuristic (Manhattan distance) to guide the search.
#  The path is reconstructed from goal to start once found, and the grid with the path is visually displayed.
#  This provides an effective way to navigate grids while avoiding obstacles.
